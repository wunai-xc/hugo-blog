...
图片放 `static/images/你的目录/` 下，正文用绝对路径引用：
...
文件夹里有 **2 篇及以上** `.md`，再加一个**空的 `_index.md`**（只需存在，内容可以是空的 `+++` `+++`）：

```
content/zh/posts/Java编程教程/
├── _index.md          ← 空文件！什么都不用写（或写空的 +++）
├── 01-Java简介.md
├── 02-基础语法.md
└── ...
```

效果：首页显示为一张**堆叠卡片**（占一个格子，右上角标篇数），点进去是该合集的文章列表。内部文章按**文件名升序**排列（01、02、03 …），上一页/下一页也按这个顺序。

> `_index.md` 想自定义合集标题就写 `title = "..."`，不写就用文件夹名。
